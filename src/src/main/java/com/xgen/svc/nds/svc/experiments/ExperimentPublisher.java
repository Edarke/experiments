...
public class ExperimentPublisher implements ConfigCallback {
  ...
  public List<ExperimentState> getExperiments(UUID clusterId, ExperimentEnv type) {
    ...
    // updates or other reads.
    // That is, consecutive calls to getExperiments always return a valid snapshot of experiments,
    // but
    // are not strictly guaranteed to provide a monotonic view of the state.
    synchronized (lock) {
    ...
    if (!pinnedClustersCopy.contains(clusterId)) {
      return cacheCopy.get(type);
    }

    // TODO: We don't need to re-allocate experiments that aren't pinned
